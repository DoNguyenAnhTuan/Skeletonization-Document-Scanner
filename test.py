...
from PIL import Image  # Thêm PIL để xử lý ảnh từ bộ nhớ
...
if uploaded_file is not None:
    try:
        # Sử dụng PIL để mở ảnh từ nội dung file tải lên (dạng BytesIO)
        img_pil = Image.open(uploaded_file)
        img_orgin = np.array(img_pil)  # Chuyển đổi ảnh từ định dạng PIL sang numpy array

        # Chuyển đổi từ RGB (PIL) sang BGR (OpenCV)
        img_orgin = cv.cvtColor(img_orgin, cv.COLOR_RGB2BGR)

        st.header("Origin")  # Hiển thị tiêu đề cho ảnh gốc
        st.image(img_orgin.astype("uint8"), channels="BGR", clamp=True)  # Hiển thị ảnh gốc

    except Exception as e:
        st.write(f"Không thể xử lý ảnh: {e}")  # Thông báo lỗi khi xử lý ảnh
else:
    # Nếu không có file tải lên, sử dụng ảnh mặc định
    img_orgin = cv.imread("./image/receipt.jpg")
    st.header("Origin (Default Image)")  # Hiển thị tiêu đề cho ảnh mặc định
    st.image(img_orgin.astype("uint8"), channels="BGR", clamp=True)  # Hiển thị ảnh mặc định
...
# Giá trị epsilon càng lớn thì đường viền được đơn giản hóa càng nhiều, điều này giúp bỏ qua các chi tiết nhỏ, chỉ giữ lại các đường viền chính.
# Ảnh hưởng trực tiếp đến bước Các điểm sau khi lược bỏ theo epsilon:
t_contour = col1_config.slider("t (epsilon = t/1000*perimeter):", min_value=1, max_value=200, step = 1, value=20)

# Nếu minval và maxval thấp, thuật toán có thể phát hiện nhiều cạnh, kể cả những cạnh nhỏ hoặc nhiễu. Ngược lại, nếu giá trị cao, thuật toán chỉ phát hiện các cạnh rõ ràng
# Các giá trị từ 0 đến 256 tương ứng với độ sáng của pixel trong ảnh xám (grayscale) (0 là đen và 255 là trắng)
